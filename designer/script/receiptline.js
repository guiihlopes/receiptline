...
(function () {
    ...
    const transform = new stream.Transform({
      ...
      transform(chunk, encoding, callback) {
        ...
        while (lines.length > 1) {
          const s = createLine(
            parseLine(lines.shift().normalize(), state),
            ptr,
            state
          );
      ...
      flush(callback) {
        ...
        const s = createLine(
          parseLine(this.data.normalize(), state),
          ptr,
          state
        );
        ptr.upsideDown ? this.buffer.push(s) : this._push(s);
  ...
  function parseOption(printer) {
    ...
    return {
      cpl: p.cpl || 48,
      encoding:
        /^(cp(437|85[28]|86[0356]|1252|93[26]|949|950)|multilingual|shiftjis|gb18030|ksc5601|big5)$/.test(
          p.encoding
        )
          ? p.encoding
          : "cp437",
      ...
      command: Object.assign(
        {},
        (typeof p.command !== "object" ? commands[p.command] : p.command) ||
          commands.svg
      ),
    };
  }
        ...
        if (/^\{[^{}]*\}$/.test(element)) {
          ...
          if (array.length === 1) {
            ...
            if ("width" in result.property) {
              const width = result.property.width
                .toLowerCase()
                .split(/[\t ]+|,/);
            ...
            if ("option" in result.property) {
              const option = result.property.option
                .toLowerCase()
                .split(/[\t ]+|,/);
              state.option = {
                type:
  ...
  function createLine(line, printer, state) {
    ...
    if (text && columns.length > 0) {
      v -=
        column.border < 0
          ? columns.length + 1
          : (columns.length - 1) * column.border;
    }
    ...
    if (text) {
      ...
      const row = column.wrap
        ? cols.reduce((a, col) => Math.max(a, col.length), 1)
        : 1;
      // sort text
      for (let j = 0; j < row; j++) {
        ...
        cols.forEach((col, i) => {
          ...
          // if wrapped text is empty
            ...
            res +=
              printer.command.normal() +
              printer.command.text(" ", printer.encoding);
          }
    ...
    column.text.forEach((text, i) => {
      ...
      if (i % 2 === 0) {
        ...
        while (t.length > 0) {
          ...
          while (j < t.length) {
            w =
              printer.command.measureText(text[j], printer.encoding) *
              [1, 1.25, 1.5, 1.75, 2, 2.5, 3, 3.5][wh];
  ...
  const _svg = {
    ...
    hr: function (width) {
      ...
      const path = `<path d="M0,${w}h${
        w * width
      }" fill="none" stroke="#000" stroke-width="2"/>`;
      this.svgContent += `<g transform="translate(${this.lineMargin * w},${
        this.svgHeight
      })">${path}</g>`;
      return "";
    },
    ...
    vrhr: function (widths1, widths2, dl, dr) {
      ...
      const path1 =
        ...
        `" fill="none" stroke="#000" stroke-width="2"/>`;
      this.svgContent += `<g transform="translate(${
        (this.lineMargin + Math.max(-dl, 0)) * w
      },${this.svgHeight})">${path1}</g>`;
      const path2 =
        `<path d="` +
        widths2
          .reduce(
      ...
      this.svgContent += `<g transform="translate(${
        (this.lineMargin + Math.max(dl, 0)) * w
      },${this.svgHeight})">${path2}</g>`;
      return "";
    },
    // set line spacing and feed new line:
    vrlf: function (vr) {
      this.feedMinimum = Number(
        this.charWidth * (!vr && this.spacing ? 2.5 : 2)
      );
      return this.lf();
    },
    // cut paper:
    cut: function () {
      const path = `<path d="M12,12.5l-7.5,-3a2,2,0,1,1,.5,0M12,11.5l-7.5,3a2,2,0,1,0,.5,0" fill="none" stroke="#000" stroke-width="1"/><path d="M12,12l10,-4q-1,-1,-2.5,-1l-10,4v2l10,4q1.5,0,2.5,-1z" fill="#000"/><path d="M24,12h${
        this.svgWidth - 24
      }" fill="none" stroke="#000" stroke-width="2" stroke-dasharray="2"/>`;
    ...
    // scale up text:
    wh: function (wh) {
      const w = [1, 1.1, 1.25, 1.5, 1.75, 2, 2.5, 3, 3.5][wh];
      const h = w;
    ...
    text: function (text, encoding) {
      ...
      this.textAttributes.x = Array.from(text)
        .map((c) => {
      ...
      this.textElement += `<text${attr}>${text.replace(
        /[ &<>]/g,
        (r) => ({ " ": "&#xa0;", "&": "&amp;", "<": "&lt;", ">": "&gt;" }[r])
      )}</text>`;
    ...
    lf: function () {
      ...
      if (this.textElement.length > 0) {
        ...
        },${this.svgHeight + h})">${this.textElement}</g>`;
      }
    ...
    qrcode: function (symbol, encoding) {
      if (typeof qrcode !== "undefined") {
        ...
        qr.make();
        qr.createSvgTag(symbol.cell, 0).replace(
          /width="(\d+)px".*height="(\d+)px".*(<path.*?>)/,
          (match, w, h, path) => {
            const margin =
              this.lineMargin * this.charWidth +
              ((this.lineWidth * this.charWidth - Number(w)) * this.lineAlign) /
                2;
    ...
    barcode: function (symbol, encoding) {
      ...
      if ("module" in bar) {
        ...
        if (symbol.hri) {
          ...
          path += `<text x="${bar.hri
            .split("")
            .map((c, i) => m + this.charWidth * i)
            .join()}" y="${height}">${bar.hri.replace(
            /[ &<>]/g,
            (r) =>
              ({ " ": "&#xa0;", "&": "&amp;", "<": "&lt;", ">": "&gt;" }[r])
          )}</text>`;
    ...
    code128a: function (x, s, d) {
      ...
      s = s.replace(
        /^((?!\d{4,})[\x00-_])+/,
        (m) => (
          m.split("").forEach((c) => d.push((c.charCodeAt(0) + 64) % 96)), ""
        )
      );
      s = s.replace(
        /^\d(?=\d{4}(\d{2})*)/,
    ...
    code93: function (data) {
      ...
      if (s.length > 0) {
        ...
        const d = s
          .split("")
          .map((c) => this.c93.escape[c.charCodeAt(0)])
          .join("")
          .split("")
          .map((c) => this.c93.code[c]);
        d.push(
          d.reduceRight((a, c, i) => a + c * (((d.length - 1 - i) % 20) + 1)) %
            47
        );
        d.push(
          d.reduceRight((a, c, i) => a + c * (((d.length - 1 - i) % 15) + 1)) %
            47
        );
      ...
      if (s.length > 0) {
        ...
        r.module =
          ...
          s
            ...
            .join("2");
        r.length =
          s.length * 25 - ((s + "$").match(/[\d\-$]/g).length - 1) * 3 - 2;
      }
      return r;
    },
    ...
    i25: {
      element:
        "22552,52225,25225,55222,22525,52522,25522,22255,52252,25252".split(
          ","
        ),
      start: "2222",
      stop: "522",
    ...
    upce: function (data) {
      ...
      if (d.length > 0) {
        ...
        d[7] =
          (10 -
            (this.upcetoa(d).reduce((a, c, i) => a + c * (3 - (i % 2) * 2), 0) %
              10)) %
          10;
        // generate HRI
        r.hri = d.join("");
    ...
    ean13: function (data) {
      ...
      if (d.length > 0) {
        ...
        d[12] = 0;
        d[12] =
          (10 - (d.reduce((a, c, i) => a + c * ((i % 2) * 2 + 1)) % 10)) % 10;
        // generate HRI
        r.hri = d.join("");
        // generate modules
        let m = this.ean.g[0];
    ...
    ean8: function (data) {
      ...
      if (d.length > 0) {
        ...
        d[7] = 0;
        d[7] =
          (10 - (d.reduce((a, c, i) => a + c * (3 - (i % 2) * 2), 0) % 10)) %
          10;
        // generate HRI
        r.hri = d.join("");
        // generate modules
  ...
  const _thermal = {
    ...
    vrstart: (widths) =>
      "\x1cC0\x1c.\x1bt\x01" +
      widths
        .reduce((a, w) => a + "\x95".repeat(w) + "\x91", "\x9c")
        .slice(0, -1) +
      "\x9d",
    ...
    vrstop: (widths) =>
      "\x1cC0\x1c.\x1bt\x01" +
      widths
        .reduce((a, w) => a + "\x95".repeat(w) + "\x90", "\x9e")
        .slice(0, -1) +
      "\x9f",
    ...
    vrlf: function (vr) {
      return (
        (vr === this.upsideDown && this.spacing ? "\x1b2" : "\x1b3\x00") +
        this.lf()
      );
    },
    // cut paper: GS V m n
    ...
    text: function (text, encoding) {
      return encoding === "multilingual"
        ...
        : this.codepage[encoding] +
            iconv.encode(text, encoding).toString("binary");
    },
    // feed new line: LF
    lf: () => "\x0a",
    // insert commands:
    ...
    image: function (image, align, left, width, right) {
      let r = this.upsideDown
        ? this.area(right, width, left) + this.align(2 - align)
        : this.area(left, width, right) + this.align(align);
    ...
    qrcode: function (symbol, encoding) {
      ...
      return (
        ...
        $(4, 0, 49, 65, 50, 0) +
        "\x1d(k" +
        $(3, 0, 49, 67, symbol.cell) +
        "\x1d(k" +
        $(3, 0, 49, 69, this.qrlevel[symbol.level]) +
        "\x1d(k" +
        $((d.length + 3) & 255, ((d.length + 3) >> 8) & 255, 49, 80, 48) +
        d +
        "\x1d(k" +
        $(3, 0, 49, 81, 48)
      );
  ...
  const _fit = {
    ...
    image: function (image, align, left, width, right) {
      let r =
        (this.upsideDown && align === 2
          ? this.area(right, width, left)
          : this.area(left, width, right)) + this.align(align);
      const img = PNG.sync.read(Buffer.from(image, "base64"));
      const w = img.width;
      const d = Array(w).fill(0);
  ...
  const _impact = {
    ...
    vr: function (widths, height) {
      const d =
        "\x1b!" + $(this.font + (height > 1 ? 16 : 0)) + "\x1bt\x01\x96";
    ...
    vrlf: function (vr) {
      return (
        (vr === this.upsideDown && this.spacing ? "\x1b2" : "\x1b3\x12") +
        this.lf()
      );
    },
    // cut paper: GS V m n
        ...
        r +=
          this.red
            ...
            .reduce((a, c) => {
              ...
              p = c.index + c.length;
              return s;
            }, "\x1br\x01\x1b!" + $(this.font) + " ".repeat(this.margin)) +
          "\x0d\x1br\x00";
      }
  ...
  const _star = {
    ...
    open: function (printer) {
      ...
      this.threshold = printer.threshold;
      return (
        "\x1b@\x1b\x1ea\x00\x1b\x1eF\x00\x1b 0\x1bs00" +
    ...
    wh: (wh) =>
      "\x1bi" + (wh < 3 ? $((wh >> 1) & 1, wh & 1) : $(wh - 2, wh - 2)),
    ...
    image: function (image, align, left, width, right) {
      ...
      for (let z = 0; z < img.height; z += this.split) {
        ...
        let r =
          "\x1b\x1dS" +
          $(1, l & 255, (l >> 8) & 255, h & 255, (h >> 8) & 255, 0);
        for (let y = 0; y < h; y++) {
          let i = 0,
            e = 0;
          for (let x = 0; x < w; x += 8) {
    ...
    barcode: function (symbol, encoding) {
      ...
      return (
        "\x1bb" +
        $(b, symbol.hri ? 50 : 49, symbol.width + 47, symbol.height) +
        d +
        "\x1e"
      );
    },
  ...
  const _sbcs = {
    ...
    vrstart: (widths) =>
      "\x1b\x1dt\x01" +
      widths
        .reduce((a, w) => a + "\xc4".repeat(w) + "\xc2", "\xda")
        .slice(0, -1) +
      "\xbf",
    ...
    vrstop: (widths) =>
      "\x1b\x1dt\x01" +
      widths
        .reduce((a, w) => a + "\xc4".repeat(w) + "\xc1", "\xc0")
        .slice(0, -1) +
      "\xd9",
  ...
  const _stargraphic = {
    ...
    open: function (printer) {
      ...
      this.threshold = printer.threshold;
      return (
        "\x1b\x1ea\x00\x1b*rA\x1b*rP0\x00" +
        (this.cutting ? "" : "\x1b*rE1\x00")
      );
    },
    ...
    image: function (image, align, left, width, right) {
      ...
      const m = Math.max(
        ((this.upsideDown ? right : left) * this.charWidth +
          (width * this.charWidth - w) *
            (this.upsideDown ? 2 - align : align)) >>
          1,
        0
      );
      const l = (m + w + 7) >> 3;
