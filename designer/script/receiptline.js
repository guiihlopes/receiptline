...
(function () {
  ...
  function createTransform(printer) {
    ...
    const transform = new stream.Transform({
      construct(callback) {
        ...
        this.data = "";
        this.encoding = /^(svg|text)$/.test(printer.command)
          ? "utf8"
          : "binary";
  ...
  function parseLine(columns, state) {
    ...
    const line = columns
      ...
      .map((column, index, array) => {
        ...
        result.align =
          1 + Number(/^[\t ]/.test(column)) - Number(/[\t ]$/.test(column));
        // parse properties
        if (/^\{[^{}]*\}$/.test(element)) {
          // extract members
    ...
    barcode: function (symbol, encoding) {
      ...
      if ("length" in bar) {
        ...
        if (bar.hri) {
          ...
          const tspan = bar.text
            .split("")
            .reduce(
              (a, c, i) =>
                a +
                `<tspan x="${m + this.charWidth * i}">${c.replace(
                  /[ &<>]/g,
                  (r) =>
                    ({ " ": "&#xa0;", "&": "&amp;", "<": "&lt;", ">": "&gt;" }[
                      r
  ...
  const barcode = {
    ...
    codabar: function (symbol) {
      ...
      if (s.length > 0) {
        ...
        r.widths = m
          .split("")
          .map((c) => (parseInt(c, 16) * symbol.width + 1) >> 1);
  ...
  const _thermal = {
        ...
        case "tis620":
          return (
            ...
            this.arrayFrom(text, encoding).reduce(
              (a, c) =>
                a + "\x00" + iconv.encode(c, encoding).toString("binary"),
              ""
            )
          );
        default:
    ...
    qrcode: function (symbol, encoding) {
      if (typeof qrcode !== "undefined") {
        let r = this.upsideDown
          ? this.area(
              this.right + this.marginRight - this.margin,
              this.width,
              this.left
  ...
  const _sii = {
    ...
    area: function (left, width, right) {
      ...
      this.right = right;
      const m =
        (this.upsideDown ? this.marginRight + right : this.margin + left) *
        this.charWidth;
      const w = width * this.charWidth;
      return (
        "\x1dL" +
  ...
  const _fit = {
    ...
    image: function (image) {
      ...
      return (
        (this.upsideDown && align === 2 ? this.area(right, width, left) : "") +
        s.join("")
      );
    },
    // print QR Code: GS ( k pL pH cn fn n1 n2 GS ( k pL pH cn fn n GS ( k pL pH cn fn n GS ( k pL pH cn fn m d1 ... dk GS ( k pL pH cn fn m
    ...
    barcode: function (symbol, encoding) {
      ...
      const u =
        symbol.type === "itf"
          ...
          : symbol.width +
            (/^(code39|codabar|nw7)$/.test(symbol.type) ? 50 : 47);
      return d.length > 0
  ...
  const _dot = {
    ...
    image: function (image) {
      ...
      for (let y = 0; y < img.height; y += 8) {
        ...
        s.push(
          "\x1bK" + $(w) + "\x00" + b.reduce((a, c) => a + $(c), "") + "\x0a"
        );
      }
      if (this.upsideDown) {
        s.reverse();
  ...
  const _stargraphic = {
    ...
    open: function (printer) {
      ...
      this.right = 0;
      this.margin =
        (printer.upsideDown ? printer.marginRight : printer.margin) *
        this.charWidth;
      return (
  ...
  const _text = {
    ...
    text: function (text, encoding) {
      const d = this.arrayFrom(text, encoding).reduce(
        (a, c) =>
          a + c + " ".repeat(this.measureText(c, encoding) * (this.scale - 1)),
        ""
      );
    ...
    qrcode: function (symbol, encoding) {
      if (typeof qrcode !== "undefined") {
        ...
        if (symbol.data.length > 0) {
          ...
          const l = (w + 7) >> 3;
          r +=
            "\x1dv0" + $(0, l & 255, (l >> 8) & 255, h & 255, (h >> 8) & 255);
          for (let i = 0; i < img.length; i++) {
            let d = "";
            for (let j = 0; j < w; j += 8) {
              let b = 0;
