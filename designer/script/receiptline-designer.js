...
function initialize() {
  ...
  saveok.onclick = (event) => {
    ...
    if (savetext.checked) {
      ...
      a.href = window.URL.createObjectURL(
        new Blob([bom, edit.value], { type: "text/plain" })
      );
    ...
    if (savesvg.checked) {
      ...
      const a = document.createElement("a");
      a.href = window.URL.createObjectURL(
        new Blob([bom, svg], { type: "image/svg+xml" })
      );
  ...
  document.querySelectorAll("figure img").forEach((el) => (el.width *= 0.75));
