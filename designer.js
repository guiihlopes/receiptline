...
if ("serial" in servers) {
  ...
  serial.listen(servers.serial.port, () => {
    console.log(
      `Serial-LAN converter running at ${servers.serial.host}:${servers.serial.port}`
    );
  });
}
...
if ("print" in servers) {
  const printer = net.createServer((conn) => {
    conn.on("data", (data) => {
      ...
      const hex = (
        data.toString("hex").replace(/../g, " $&").replace(/.{24}/g, "$& ") +
        " ".repeat(49)
      ).match(/.{50}/g);
      const bin = (
        data.toString("binary").replace(/[^ -~]/g, ".") + " ".repeat(15)
      ).match(/.{16}/g);
      bin.forEach((b, i) =>
        console.log(
          `${("0".repeat(7) + (i << 4).toString(16)).slice(-8)} ${hex[i]} ${b}`
        )
      );
      conn.write("\x00");
    });
  ...
  printer.listen(servers.print.port, () => {
    console.log(
      `Virtual printer running at ${servers.print.host}:${servers.print.port}`
    );
  });
}
...
if ("http" in servers) {
      ...
      case "GET":
        ...
        fs.readFile(servers.http.root + pathname, (err, data) => {
          if (err) {
            ...
            res.writeHead(200, {
              "Content-Type":
                servers.http.mime[path.extname(pathname)] ||
                servers.http.mime[".txt"],
            });
            res.end(data);
          }
        });
              ...
              req.on("end", () => {
                ...
                sock.on("connect", () => {
                  transform(text, printer).then((command) => {
                    drain = sock.write(
                      command,
                      /^(svg|text)$/.test(printer.command) ? "utf8" : "binary"
                    );
                  });
                });
                sock.on("data", (data) => {
  ...
  server.listen(servers.http.port, servers.http.host, () => {
    console.log(
      `Server running at http://${servers.http.host}:${servers.http.port}/`
    );
  });
}
...
const transform = async (receiptmd, printer) => {
  ...
  if (
    printer.landscape &&
    /^(escpos|epson|sii|citizen|star[sm]bcs2?)$/.test(printer.command)
  ) {
    // landscape orientation
    printer.command = Object.assign(
      {},
      receiptline.commands[printer.command],
      ...landscape[printer.command]
    );
  }
  return receiptline.transform(receiptmd, printer);
};
...
const rasterize = async (receiptmd, printer, encoding) => {
  ...
  if (puppeteer) {
    ...
    await page.setContent(
      `<!DOCTYPE html><html><head><meta charset="utf-8"><style>*{margin:0;background:transparent}${t}</style></head><body>${svg}</body></html>`
    );
...
const svgsharp = Object.assign({}, receiptline.commands.svg, {
  ...
  text: function (text, encoding) {
    ...
    this.textElement += this.arrayFrom(text, encoding).reduce((a, c) => {
      ...
      const t = attr
        .replace("scale(1,2)", `translate(${s}),scale(1,2)`)
        .replace("scale(2,1)", `translate(${-s}),scale(2,1)`);
      return (
        a +
        `<text x="${r}"${t}>${c.replace(
          /[ &<>]/g,
          (r) => ({ " ": "&#xa0;", "&": "&amp;", "<": "&lt;", ">": "&gt;" }[r])
        )}</text>`
      );
  ...
  lf: function () {
    ...
    if (this.textElement.length > 0) {
      ...
      },${this.svgHeight + (h * 5) / 6})">${this.textElement}</g>`;
    }
...
const _escpos90 = {
  ...
  open: function (printer) {
    ...
    return (
      ...
      "\x1c(A" +
      $(2, 0, 48, 0) +
      "\x1b \x00\x1cS\x00\x00\x1c.\x1dP" +
      $(r, r) +
      "\x1bL\x1bT" +
      $(printer.upsideDown ? 3 : 1)
    );
  },
  ...
  close: function () {
    ...
    const m =
      (this.upsideDown ? this.margin : this.marginRight) * this.charWidth;
    return (
      "\x1bW" +
      $(0, 0, 0, 0, w & 255, (w >> 8) & 255, v & 255, (v >> 8) & 255) +
      " \x1bW" +
      $(
        0,
        0,
  ...
  image: function (image) {
    ...
    const w = img.width;
    const x =
      left * this.charWidth + (align * (width * this.charWidth - w)) / 2;
  ...
  qrcode: function (symbol, encoding) {
    if (typeof qrcode !== "undefined" && symbol.data.length > 0) {
      ...
      const h = w;
      const x =
        this.left * this.charWidth +
        (this.alignment * (this.width * this.charWidth - w)) / 2;
      const y = this.position;
      let r =
        "\x1d$" +
        $((y + h - 1) & 255, ((y + h - 1) >> 8) & 255) +
...
const _sii90 = {
  ...
  open: function (printer) {
    ...
    const r = printer.resolution;
    return (
      "\x1b@\x1da\x00\x1bM0\x1b \x00\x1cS\x00\x00\x1c.\x1dP" +
      $(r, r) +
      "\x1bL\x1bT" +
      $(printer.upsideDown ? 3 : 1)
    );
...
const _star90 = {
  ...
  open: function (printer) {
    ...
    return (
      ...
      "\x1b\x1eF\x00\x1b 0\x1bs00\x1b\x1dP0\x1b\x1dP2" +
      $(printer.upsideDown ? 3 : 1)
    );
  },
...
const _mbcs290 = {
  ...
  vrhr: function (widths1, widths2, dl, dr) {
    ...
    this.content += r2
      .split("")
      .reduce((a, c, i) => a + this.vrtable[c][r1[i]], "");
    return "";
  },
};
